...
2. 상단 헤더 영역 높이: 56 (back button row).
3. 진행바(progress)는 헤더 아래 24~32 간격 후 노출.
4. **고정 하단 CTA**가 있는 화면은 콘텐츠에 `pb-32` 이상 부여하여 가림 방지.
5. 카드 간 세로 간격: `space-y-4`(16) 또는 `space-y-6`(24) 두 가지로만.
...
- 홈은 back 없이 좌측에 사용자 이름(`text-xl font-bold`) + 우측 bell/menu 아이콘.
